class BooksController < ApplicationController
  ...
  def send_notification(user_notification_dest,subscriber_notification_dest, book_name, user_id, subscriber_id)
    fcm = FCM.new("AIzaSyC7EB-g9d49wRC-Ki7UiPy5qry0QOWw4SE")
    registration_ids= [user_notification_dest] # an array of one or more client registration tokens
    options = {notification: {body: "تم طلب الكتاب -#{book_name}- للاستعارة"}}
    puts response = fcm.send(registration_ids, options)


    registration_ids= [subscriber_notification_dest] # an array of one or more client registration tokens
    options = {notification: {body: "تم حجز الكتاب -#{book_name}- للاستعارة"}}
    puts response = fcm.send(registration_ids, options)

    Notification.create(title: "كتاب مطلوب للاستعارة", content: "تم طلب الكتاب -#{book_name}- للاستعارة", date: Date.today, user_id: user_id)
    Notification.create(title: "استعارة كتاب", content: "تم حجز الكتاب -#{book_name}- للاستعارة", date: Date.today, user_id: subscriber_id)
  end
